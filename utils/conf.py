...
def get_config():
    ...
    if cfg.dataset == 'iNaturalist18' or cfg.dataset == 'places365':
        cfg['pretrained'] = 0
        cfg['lr_group'] = [cfg.lr, cfg.lr] # [0.1,0.1] for batch_size=256
        cfg['lrstep'] = 'none'
...
# Our DTRG parameters
parser.add_argument('--start_dtrg', default=1, type=int, metavar='N', help='start epoch for DTRG')
parser.add_argument('--ocl', dest='ocl', action='store_true', help='online center loss')
parser.add_argument('--weight_cent', type=float, default=1e-3, help="weight for center loss")

parser.add_argument('--graph', dest='graph', action='store_true', help='graph')
parser.add_argument('--distmethod', default='eu', type=str, help='similarity distance: euclidean, kl')
parser.add_argument('--tau', type=float, default=1, help="the temperature parameter for kernel function, kl_t:0.1, eu_t:1")
parser.add_argument('--eta', type=float, default=1, help="weight for similarity distance, kl_t:5, eu_t:1")

# label smooth parameter
parser.add_argument('--ls', dest='ls', action='store_true', help='label smooth')

# online label smooth parameter
parser.add_argument('--ols', dest='ols', action='store_true', help='online label smooth')

# center loss parameters
parser.add_argument('--cl', dest='cl', action='store_true', help='center loss')
parser.add_argument('--lr_cent', type=float, default=0.5, help="learning rate for center loss")

# parameter partition for dtd dataset
parser.add_argument('--partition', type=int, default=1, help="partition for split  with `1 <= partition <= 10`")
